...
// clang-format off
        ...
        case KVM_EXIT_DEBUG:
            kvm_arch_get_registers_fast(cpu);
            // cpu_dump_state(cpu, stdout, CPU_DUMP_CODE);
            X86CPU      *x86_cpu = X86_CPU(cpu);
            CPUX86State *env     = &x86_cpu->env;
            uint64_t current_rip = get_rip(cpu);
            // dbg_printf("\n[KVM_EXIT_DEBUG] Handling DBG Event\n");
            // dbg_printf("[KVM_EXIT_DEBUG] Current-PC: %p\n", current_rip);
            // Check if is critical bp
            if(current_rip == GET_CUSTOM_HEADER()->presnap_lock_bp){
                dbg_printf("[KVM_EXIT_DEBUG] Handling presnap hook: %p\n", current_rip);
                // Before take presnap, we must restore the breakpoint first
                restore_breakpoint(current_rip, cpu);
                // Issue a hypercall for locking presnap...
                custom_kafl_lock(run, cpu, 0); // Lock pre snap
                ret = 0;
                break;
            }
            else if(current_rip == GET_CUSTOM_HEADER()->prepare_bp){
                dbg_printf("[KVM_EXIT_DEBUG] Handling prepare stage of bp: %p\n", current_rip);
                restore_breakpoint(current_rip, cpu);
                // We put lateral steps here for current time being.
                // initial_fuzzer_handshake (pt has been set using a hypercall once in all)
                init_fuzzer_handshake(run, cpu, 0);
                // 
                custom_kafl_print(run, cpu, input_buffer_addr); // enable output.
                // get host configs
                custom_kafl_get_host_config(run, cpu, 0);
                // set guest configs
                custom_kafl_set_guest_config(run, cpu, 0);
                // remapping the payload buffer
                custom_kafl_set_guest_payload_buffer(run, cpu, 0);
                // set pr cr3 = 0
                custom_kafl_set_pt_cr3(run, cpu, 0);
                // reset pt
                custom_kafl_reset_pt_configs(run, cpu, 0);
                // reset hw bp after loading from lock bp
                dbg_printf("[+] Recovering bps\n");
                
                kvm_insert_breakpoint_hw(cpu, GET_CUSTOM_HEADER()->end_fuzzing_loop_bp, 1, 1);
                kvm_insert_breakpoint_hw(cpu, GET_CUSTOM_HEADER()->enter_fuzzing_loop_bp, 1, 1);
                ...
                // enable_singlestep_debug(cpu);
                // handle_hypercall_kafl_next_payload(run, cpu, 0);
                // test_update_cpu(cpu);
                ret = 0;
                break;
            }
            else if (current_rip == GET_CUSTOM_HEADER()->enter_fuzzing_loop_bp){
                // dbg_printf("[KVM_EXIT_DEBUG] Handling enter_fuzzing_loop_bp: %p\n", current_rip);
                
                // custom_kafl_fast_acquire(run, cpu, 0);
                // restore_breakpoint(current_rip, cpu);
                handle_hypercall_kafl_next_payload_breakpoint(run, cpu, 0);
                
                // dbg_printf("[KVM_EXIT_DEBUG] enter_fuzzing_loop_bp done\n\n");
                ...
                // hexdump_virtual_memory(input_buffer_addr + 4, 20, cpu);
                if(setup_once){
                    handle_hypercall_kafl_acquire(run, cpu, 0);
                    kvm_remove_breakpoint_hw(cpu, GET_CUSTOM_HEADER()->enter_fuzzing_loop_bp, 1, 1);
                    custom_kafl_copy_payload_buffer(run, cpu, input_buffer_addr, env->regs[R_ECX]);
                    // env->regs[R_ECX] = dup_payload_buffer; // better copy a buffer.
                    // custom_kafl_reset_rcx(cpu, dup_payload_buffer);
                    // handle_hypercall_kafl_panic(run, cpu, 0);
                }
                setup_once = true;
                kvm_insert_breakpoint_hw(cpu, GET_CUSTOM_HEADER()->end_fuzzing_loop_bp, 1, 1);
            ...
            else if(current_rip == GET_CUSTOM_HEADER()->end_fuzzing_loop_bp){
                // restore_breakpoint(current_rip, cpu);
                // dbg_printf("[KVM_EXIT_DEBUG] Handling end_fuzzing_loop_bp: %p\n", current_rip);
                custom_kafl_leave_fuzz_loop(run, cpu, 0);

                kvm_remove_breakpoint_hw(cpu, GET_CUSTOM_HEADER()->end_fuzzing_loop_bp, 1, 1);
                kvm_insert_breakpoint_hw(cpu, GET_CUSTOM_HEADER()->enter_fuzzing_loop_bp, 1, 1);
                ret = 0;
                break;
            }
...
int kvm_update_guest_debug_hw(CPUState *cpu, unsigned long reinject_trap)
{
    struct kvm_set_guest_debug_data data;
    ...
    if (cpu->singlestep_enabled) {
        // data.dbg.control |= KVM_GUESTDBG_ENABLE | KVM_GUESTDBG_SINGLESTEP;
        data.dbg.control |= KVM_GUESTDBG_ENABLE;
    }
    kvm_arch_update_guest_debug_hw(cpu, &data.dbg);
...
int forcpu_update_guest_debug(CPUState *cpu)
{
    if(cpu_debug_enabled)
    {
        printf("[+] Maintaining current CPU Status.\n");
        return 0;
    }else{
        printf("[+] Updating CPU Status.\n");
        struct kvm_set_guest_debug_data data;
...
int flush_update_guest_cpu_debug(CPUState *cpu)
{
    printf("[+] Updating the CPU Status.\n");
    struct kvm_set_guest_debug_data data;
    data.dbg.control = 0;
    data.dbg.control |= KVM_GUESTDBG_ENABLE | KVM_GUESTDBG_USE_SW_BP | KVM_GUESTDBG_USE_HW_BP;
    return kvm_vcpu_ioctl(cpu, KVM_SET_GUEST_DEBUG, &data.dbg);
}

int test_update_cpu(CPUState *cpu)
{
    struct kvm_set_guest_debug_data data;
...
int enable_singlestep_debug(CPUState *cpu)
{
        ...
        data.dbg.control |= KVM_GUESTDBG_SINGLESTEP | KVM_GUESTDBG_ENABLE;

        return kvm_vcpu_ioctl(cpu, KVM_SET_GUEST_DEBUG, &data.dbg);
}

int kvm_insert_breakpoint_hw(CPUState *cpu, target_ulong addr,
                          target_ulong len, int type)
{
    ...
    CPU_FOREACH(cpu) {
        err = kvm_update_guest_debug_hw(cpu, 0);
        if (err) {
            return err;
        }
    }
...
int kvm_remove_breakpoint_hw(CPUState *cpu, target_ulong addr,
                          target_ulong len, int type)
{
    struct kvm_sw_breakpoint *bp;
    int err;
