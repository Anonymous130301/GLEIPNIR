...
{
    nyx_trace();
    dbg_printf("[+] kafl_next_payload invoked, current init_state: %d! \n", init_state);
    if (hypercall_enabled) {
        if (init_state) {
            dbg_printf("[+] kafl_next_payload invoked! \n");
            ...
            if (!setup_snapshot_once) {
                ...
                GET_GLOBAL_STATE()->in_fuzzing_mode = true;
                // check if edgecov trace is enabled.
...
bool handle_hypercall_kafl_next_payload_breakpoint(struct kvm_run *run,
                                        CPUState       *cpu,
                                        uint64_t        hypercall_arg)
{
    nyx_trace();
    dbg_printf("[+] kafl_next_payload invoked, current state: %d! \n", init_state);
    if (hypercall_enabled) {
        if (init_state) {
            set_state_auxiliary_result_buffer(GET_GLOBAL_STATE()->auxilary_buffer, 2);
            synchronization_lock();
...
static void acquire_print_once(CPUState *cpu)
{
    if (acquire_print_once_bool) {
        ...
        printf("handle_hypercall_kafl_acquire at IP: %lx\n", get_rip(cpu));
    }
}
                                   ...
                                   uint64_t        hypercall_arg)
{
    if (hypercall_enabled) {
        if (!init_state) {
            dbg_printf("[+] handle_hypercall_kafl_acquire.\n");
            acquire_print_once(cpu);
            synchronization_enter_fuzzing_loop(cpu);
        }
    }
    // get_current_payload(cpu);
}
                                   ...
                                   uint64_t        hypercall_arg)
{
    if (hypercall_enabled) {
        if (init_state) {
            dbg_printf("[RELEASE] init_state=false\n");
            init_state = false;
        } else {
            if (hypercall_arg > 0) {
            ...
            dbg_printf("[RELEASE] execute synchronization_disable_pt\n");
                                                 ...
                                                 uint64_t        hypercall_arg)
{
    ...
    dbg_printf("[PANIC EXTENDED] message from vm: %s is_fuzzingmode: %d is_existed: %d\n", (uint8_t *)GET_GLOBAL_STATE()->hprintf_tmp_buffer, GET_GLOBAL_STATE()->in_fuzzing_mode, fast_reload_snapshot_exists(get_fast_reload_snapshot())); 
                          ...
                          uint64_t        arg)
{
    ...
    case KVM_EXIT_KAFL_PERSIST_PAGE_PAST_SNAPSHOT:
        ...
        ret = 0;
        break;
    case KVM_EXIT_ENTER_FUZZING_LOOP_BREAKPOINT:
        handle_hypercall_enter_fuzzing_loop_breakpoint(run, cpu, arg);
        ret = 0;
        break;
    case KVM_EXIT_LEAVE_FUZZING_LOOP_BREAKPOINT:
        handle_hypercall_leave_fuzzing_loop_breakpoint(run, cpu, arg);
        ret = 0;
        break;
    case KVM_EXIT_SET_KVM_BREAKPOINT:
        handle_hypercall_set_kvm_breakpoint(run, cpu, arg);
        ret = 0;
        break;
    case KVM_EXIT_SUBMIT_CLIENT_CR3:
        handle_hypercall_submit_client_cr3(run, cpu, arg);
        ret = 0;
        break;
    case KVM_EXIT_SET_PRESNAP_BREAKPOINT:
        handle_hypercall_set_presnap_breakpoint(run, cpu, arg);
        ret = 0;
        break;
    case KVM_EXIT_SET_CLIENT_PT_RANGE:
        handle_hypercall_set_client_pt_range(run, cpu, arg);
        ret = 0;
        break;
    case KVM_EXIT_SET_PREPARE_BREAKPOINT:
        handle_hypercall_set_prepare_breakpoint(run, cpu, arg);
        ret = 0;
        break;
    case KVM_EXIT_TEST_PRINTF:
        handle_hypercall_test_output(run, cpu, arg);
        ret = 0;
        break;
    case KVM_EXIT_NEXTPAYLOAD_BREAKPOINT:
        handle_hypercall_nextpayload_breakpoint(run, cpu, arg);
        ret = 0;
        break;
    case KVM_EXIT_SET_KVM_HW_BREAKPOINT:
        handle_hypercall_set_kvm_hw_breakpoint(run, cpu, arg);
        ret = 0;
        break;
    }
    return ret;
