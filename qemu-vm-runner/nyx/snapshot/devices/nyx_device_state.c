...
static void fast_qemu_savevm_state_complete_precopy(QEMUFile *f, bool iterable_only)
{
    ...
    QTAILQ_FOREACH (se, &savevm_state.handlers, entry) {
        if (strcmp(se->idstr, "ram") && strcmp(se->idstr, "block") && strcmp(se->idstr, "slirp") && !strstr(se->idstr, "e1000") && !strstr(se->idstr, "uhci") && !strstr(se->idstr, "usb")) {
            printf("[+] fast qemu savevm: Processing device: [%s]\n", se->idstr);
